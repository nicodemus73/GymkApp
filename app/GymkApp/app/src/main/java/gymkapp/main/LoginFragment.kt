  ...
  override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
    ...
    view.inputUsername.editText?.doAfterTextChanged {
      view.inputUsername.error =
        loginFragmentModel.validateUsername(view.inputUsername.editText?.text.toString())
    }
    view.inputPassword.editText?.doAfterTextChanged {
      view.inputPassword.error =
        loginFragmentModel.validatePassword(view.inputPassword.editText?.text.toString())
    }
    ...
    viewModel.authenticationState.observe(viewLifecycleOwner, Observer { authState ->
      if (authState == INVALID_AUTHENTICATION) {
        Log.d(javaClass.name, "Error al autenticar, se imprimira el mensaje")
        errorSnackBar =
          Snackbar.make(view, viewModel.errorMessage, Snackbar.LENGTH_SHORT).setAction("Ignore") {}
        errorSnackBar.show()
      }
    })
  }
